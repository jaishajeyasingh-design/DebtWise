...
import { Check, Shield, Clock, AlertCircle, ArrowRight, UserCheck, Star } from 'lucide-react';
...
export default function InterventionCard({
  ...
  return (
    ...
    >
      ...
      {is_recommended && (
        <div className="absolute -top-3 left-6 px-3 py-0.5 rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-mono text-[11px] font-bold tracking-wider flex items-center gap-1 shadow-md">
      ...
      <div>
        ...
        {intervention.components && intervention.components.length > 0 && (
          <div className="space-y-2.5 my-4 p-4 rounded-xl bg-slate-950/60 border border-slate-800">
        ...
        {intervention.policy_note && (
          <div className="p-2.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-xs text-amber-200/90 font-mono mb-3 flex items-start gap-2">
      ...
      <div className="mt-2 pt-2 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="text-2xl font-mono font-black text-emerald-400">
            {monthly_payment_after > 0 ? `₹${monthly_payment_after.toLocaleString('en-IN')}` : "Custom Case Review"}
            {monthly_payment_after > 0 && <span className="text-xs font-normal text-slate-400 font-sans"> / mo</span>}
          </div>
        </div>
